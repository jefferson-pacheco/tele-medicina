...
def cadastro_medico(request):
    if is_medico(request.user):
        messages.add_message(request, messages.WARNING, 'Você já é médico')
...
def abrir_horario(request):
    if not is_medico(request.user):
        messages.add_message(request, messages.WARNING, 'Somente médicos podem abrir horário!')
    ...
    if request.method == "GET":
        ...
        if data_formatada <= datetime.now():
            messages.add_message(request, messages.WARNING, 'A data não pode ser anterior à data atual')
            return redirect('/medicos/abrir_horario')
...
def consulta_area_medico(request, id_consulta):
    if not is_medico(request.user):
        messages.add_message(request, messages.WARNING, 'Somente médicos podem acessar essa página.')
        return redirect('/usuarios/sair')
    ...
    if request.method == "GET":
        ...
        if consulta.status == 'C' or consulta.status == "F":
            messages.add_message(request, messages.WARNING, 'Essa consulta já foi cancelada ou finalizada.')
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
         messages.add_message(request, messages.ERROR, 'Essa consulta não é sua!')
         return redirect(f'/medicos/consulta_area_medico/{id_consulta}')
...
def add_documento(request, consulta_id):
    ...
    if request.method == "POST":
        ...
        if not documento:
            messages.add_message(request, messages.WARNING, 'Adicione o documento.')
            return redirect(f'/medicos/consulta_area_medico/{consulta_id}')
